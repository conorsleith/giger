...
class SliderPair:
    def __init__(self, master, logscale=False, callback=None):
        self._int_slider = CTkSlider(
            master=master, from_=0, to=100, number_of_steps=100
        )
        self._float_slider = CTkSlider(
            master=master, from_=0, to=100, number_of_steps=100
        )
...
class HRTrainer(customtkinter.CTk):
    def __init__(self):
        ...
        self._giger = controller.Giger(
            None,
            None,
            max_power=STARTING_MAX_WATTS_VALUE,
            min_power=STARTING_MIN_WATTS_VALUE,
    ...
    def _get_measurements(self) -> Tuple[Measurement, Measurement, Measurement]:
        ...
        ts = time()
        return (
            Measurement(ts, self._giger.current_hr),
            Measurement(ts, self._giger.current_trainer_power),
            Measurement(ts, self._giger.hr_setpoint),
        )
    ...
    def _open_device_picker(self):
        ...
        if (
            self._device_picker_window is None
            or not self._device_picker_window.winfo_exists()
        ):
            self._device_picker_window = DevicePicker(
                loop=self._loop, done_callback=self._change_devices
            )
    ...
    def _setup_ui(self):
        ...
        self._bottom_frame.pack(
            pady=10, padx=10, expand=True, side="bottom", fill="both"
        )

        self._cycling_frame = CTkFrame(master=self._top_frame)
        self._cycling_frame.pack(
            pady=10, padx=10, fill="both", expand=False, side="left"
        )

        self._weights_favorites_tab = CTkTabview(master=self._top_frame)
        self._weights_favorites_tab.pack(
            pady=10, padx=10, fill="both", expand=False, side="left"
        )
        ...
        self._kweights_frame_label = CTkLabel(
            master=self._kweights_frame, text="K weights", justify="left"
        )
        ...
        for idx, watts in enumerate(reversed(range(150, 450, 25))):
            ...
            column = idx % 2
            button = CTkButton(
                master=self._watt_favorites_frame,
                text=f"{watts}",
                command=callback_factory(watts),
            )
            button.grid(row=idx // 2, column=column, padx=5, pady=10)

        updown_row = (idx + 1) // 2
        self._watts_down_button = CTkButton(
            master=self._watt_favorites_frame,
            text="-10",
            command=self._watts_down_callback,
        )
        self._watts_down_button.grid(row=updown_row, column=0, padx=5, pady=10)
        self._watts_up_button = CTkButton(
            master=self._watt_favorites_frame,
            text="+10",
            command=self._watts_up_callback,
        )
        ...
        for idx, hr in enumerate(reversed([130, 140, 150, 160, 170, 180])):
            ...
            button = CTkButton(
                master=self._hr_favorites_frame,
                text=f"{hr}",
                command=callback_factory(hr),
            )
        ...
        self._cycling_metrics_frame.pack(
            pady=10, padx=10, fill="both", expand=True, side="top"
        )

        self._control_frame = CTkFrame(master=self._cycling_frame)
        self._control_frame.pack(
            pady=10, padx=10, fill="both", expand=False, side="top"
        )
        ...
        logger.add(
            self._log_box_handler,  # type: ignore
            format=(
        ...
        self._hr_setpoint_label = CTkLabel(
            master=self._cycling_metrics_frame, text="Heart Rate Setpoint"
        )
        self._hr_setpoint_slider = CTkSlider(
            master=self._cycling_metrics_frame, from_=60, to=200
        )
        self._hr_setpoint_value_label = CTkLabel(
            master=self._cycling_metrics_frame, text=f"{STARTING_HR_SETPOINT_VALUE}"
        )
        self._hr_setpoint_slider.set(STARTING_HR_SETPOINT_VALUE)
        self._hr_setpoint_slider.configure(
            command=lambda val: self._hr_setpoint_value_label.configure(
                text=f"{val:.0f}"
            )
        )

        self._min_watts_label = CTkLabel(
            master=self._cycling_metrics_frame, text="Min Watts"
        )
        self._min_watts_slider = CTkSlider(
            master=self._cycling_metrics_frame, from_=0, to=600
        )
        self._min_watts_value_label = CTkLabel(
            master=self._cycling_metrics_frame, text=f"{STARTING_MIN_WATTS_VALUE}"
        )
        self._min_watts_slider.set(STARTING_MIN_WATTS_VALUE)
        self._min_watts_slider.configure(
            command=lambda val: self._min_watts_value_label.configure(text=f"{val:.0f}")
        )

        self._max_watts_label = CTkLabel(
            master=self._cycling_metrics_frame, text="Max Watts"
        )
        self._max_watts_slider = CTkSlider(
            master=self._cycling_metrics_frame, from_=0, to=600
        )
        self._max_watts_value_label = CTkLabel(
            master=self._cycling_metrics_frame, text=f"{STARTING_MAX_WATTS_VALUE}"
        )
        self._max_watts_slider.set(STARTING_MAX_WATTS_VALUE)
        self._max_watts_slider.configure(
            command=lambda val: self._max_watts_value_label.configure(text=f"{val:.0f}")
        )

        self._set_current_watts_label = CTkLabel(
            master=self._cycling_metrics_frame, text="Set Current Watts"
        )
        self._set_current_watts_slider = CTkSlider(
            master=self._cycling_metrics_frame, from_=0, to=600
        )
        self._set_current_watts_value_label = CTkLabel(
            master=self._cycling_metrics_frame, text=f"{STARTING_MIN_WATTS_VALUE}"
        )
        self._set_current_watts_slider.set(STARTING_MIN_WATTS_VALUE)
        self._set_current_watts_slider.configure(
            command=lambda val: self._set_current_watts_value_label.configure(
                text=f"{val:.0f}"
            )
        )

        self._current_hr_label = CTkLabel(
            master=self._cycling_metrics_frame, text="Current Heart Rate"
        )
        self._current_hr_value_label = CTkLabel(
            master=self._cycling_metrics_frame, text="0"
        )

        self._current_watts_label = CTkLabel(
            master=self._cycling_metrics_frame, text="Current Watts"
        )
        self._current_watts_value_label = CTkLabel(
            master=self._cycling_metrics_frame, text="0"
        )

        self._grid_metrics_slider_group(
            0,
            self._hr_setpoint_label,
            self._hr_setpoint_slider,
            self._hr_setpoint_value_label,
        )
        self._grid_metrics_slider_group(
            1,
            self._min_watts_label,
            self._min_watts_slider,
            self._min_watts_value_label,
        )
        self._grid_metrics_slider_group(
            2,
            self._max_watts_label,
            self._max_watts_slider,
            self._max_watts_value_label,
        )
        self._grid_metrics_slider_group(
            3,
            self._set_current_watts_label,
            self._set_current_watts_slider,
            self._set_current_watts_value_label,
        )
        self._grid_metrics_label_group(
            4, self._current_hr_label, self._current_hr_value_label
        )
        self._grid_metrics_label_group(
            5, self._current_watts_label, self._current_watts_value_label
        )

        self._device_picker_button = CTkButton(
            master=self._cycling_metrics_frame, text="Devices"
        )
        ...
        self._show_graph_switch = CTkSwitch(
            master=self._control_frame, text="Show graph"
        )
        ...
        self._hr_setpoint_slider.configure(
            command=lambda value: self._hr_setpoint_callback(value)
        )
        self._min_watts_slider.configure(
            command=lambda value: self._min_watts_callback(value)
        )
        self._max_watts_slider.configure(
            command=lambda value: self._max_watts_callback(value)
        )
        self._set_current_watts_slider.bind(
            "<ButtonRelease-1>", self._set_current_watts_callback
        )
    ...
    def run(self):
        thread = Thread(
            target=self._loop.run_until_complete, args=(self._run_controller(),)
        )
        thread.start()
        self.focus_force()
        self.attributes("-topmost", self._topmost)
